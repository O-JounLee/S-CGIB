...
def main():
	...
	if args.dataset in ["ogbg-molpcba", "ogbg-molhiv", "ogbg-ppa","ogbg-code2"]:
		...
		dataset = ZINC(root='original_datasets/ZINC', subset = True)
		args.num_classes = dataset.num_classes
		args.num_features = dataset.num_features
		num_features = args.num_features
		num_classes = args.num_classes
	elif args.dataset in ["ENZYMES", "PROTEINS", "NCI1", "NCI109"]:
...
def generate_graphs(dataset, k_hop):
	...
	# 	torch.save({"B": B_lists},"pts/"+ args.dataset + "_B.pt")
	# 	torch.save({"adj": adj_list},"pts/"+ args.dataset + "_adj.pt")
	# 	torch.save({"sim": sim_list},"pts/"+ args.dataset + "_sim.pt")
	# 	torch.save({"phi": phi_list},"pts/"+ args.dataset + "_phi.pt")
	return graph_ds 
################ checking 
