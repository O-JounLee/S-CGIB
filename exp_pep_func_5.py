...
from train_pep_func import train_epoch, evaluate_network, train_epoch_graph_classification, train_epoch_domainadaptation
...
def run_epoch_graph_classification(model, train_loader, val_loader, test_loader, num_features, file_name, batch_size):
	...
	for epoch in range(1, args.ft_epoches):
		...
		if epoch % 1 == 0:
			if args.task == "graph_regression":
				print(
					f'Epoch: {epoch}	|Best_epoch: {best_epoch}	|Train_loss: {np.round(epoch_train_loss, 6)}	|Val_loss: {np.round(epoch_val_loss, 6)}	| Train_ap: {np.round(epoch_train_auc, 6)}	| Val_ap: {np.round(epoch_val_auc, 6)}	| epoch_test_ap: {np.round(epoch_test_auc, 6)} ')
		...
		if time.time() - t0 > 48 * 3600:
			...
			break
	_, test_ap = evaluate_network(args, best_model, optimizer, device, test_loader, epoch,
	...
	print("Train AP: {:.4f}".format(train_auc))
	print("Test AP: {:.4f}".format(test_auc))
	print("Convergence Time (Epochs): {:.4f}".format(epoch))
	print("TOTAL TIME TAKEN: {:.4f}s".format(time.time() - t0))
	print("Convergence Time (Epochs): {:.4f}".format(epoch))
	print("AVG TIME PER EPOCH: {:.4f}s".format(np.mean(per_epoch_time)))
	return test_ap.cpu(), best_epoch
...
from torch_geometric.datasets import TUDataset, ZINC, LRGBDataset
...
def main():
	...
	if args.dataset in ["Peptides-func"]:
		dataset_train = LRGBDataset(root='original_datasets/' + args.dataset, name=args.dataset, split='train')
		...
		if not os.path.exists(path):
			...
			graph_ds  = generate_graphs(  )
			for i in range(1,6):
				save_graphs("pts/" + args.dataset + "_k_transition_" + str(i) + ".bin", graph_ds.graph_lists, graph_ds.graph_labels)
...
def generate_graphs():
	...
	for ds in dataset_list:
		...
		dataset  = LRGBDataset(root='original_datasets/' + args.dataset, name=args.dataset, split= ds)
		for i in range(len(dataset)):
			...
			try:
				...
				all_subgraphs3 = [dgl.khop_in_subgraph(g, individual_node, k=3)[0] for individual_node in node_ids]
				set_subgraphs3.append(all_subgraphs3)

				all_subgraphs4 = [dgl.khop_in_subgraph(g, individual_node, k=4)[0] for individual_node in node_ids]
				set_subgraphs4.append(all_subgraphs4)

				all_subgraphs5 = [dgl.khop_in_subgraph(g, individual_node, k=5)[0] for individual_node in node_ids]
				set_subgraphs5.append(all_subgraphs5)
			except:
				miss += 1
	...
	torch.save({"set_subgraphs": set_subgraphs3}, "pts/" + args.dataset +  "_subgraphs_khop_" + str(3) + ".pt")
	torch.save({"set_subgraphs": set_subgraphs4}, "pts/" + args.dataset +  "_subgraphs_khop_" + str(4) + ".pt")
	torch.save({"set_subgraphs": set_subgraphs5}, "pts/" + args.dataset +  "_subgraphs_khop_" + str(5) + ".pt")

	torch.save({"trans_logMs": trans_logMs}, "pts/" + args.dataset +  "_M_khop_" + str(1) + ".pt")
	torch.save({"trans_logMs": trans_logMs}, "pts/" + args.dataset +  "_M_khop_" + str(2) + ".pt")
	torch.save({"trans_logMs": trans_logMs}, "pts/" + args.dataset + "_M_khop_" + str(3) + ".pt")
	torch.save({"trans_logMs": trans_logMs}, "pts/" + args.dataset + "_M_khop_" + str(4) + ".pt")
	torch.save({"trans_logMs": trans_logMs}, "pts/" + args.dataset + "_M_khop_" + str(5) + ".pt")
	return graph_ds
...
# 			graph_labels += data.y		#graph_labels.append(data.y)
# 			node_ids = g.nodes()
...
if __name__ == '__main__':
	...
	#
	parser.add_argument("--dataset", default="Peptides-func", help="Dataset")
