...
def train(model, train_data, mask, optimizer, device, device_2, g, adj_org, trans_logM, sim, phi, B, k_transition, current_epoch,
    ...
    loss_all = param_1 * loss_M + param_2 * loss_X + param_3 * adj_loss
...
def check_degree(test_data, pred, degree, mask):
    n_node_check = 50
    d_dict = {}
    d_dict = defaultdict(lambda: 0, d_dict)
    degree_frequency = collections.Counter(degree)
    degree_frequency = dict(sorted(degree_frequency.items()))
    degree_frequency = {int(k): int(v) for k, v in degree_frequency.items()}

    y = test_data.y
    # print(f'y size: {y.size()}: val: {y}')
    # print(f'mask size: {mask.size()}:  {mask}'  )

    # count node -degree in test data:
    degree_dict_test = {}
    degree_dict_test = defaultdict(lambda: 0, degree_dict_test)
    for i in range(len(y)):
        if mask[i] == True:
            node_degree = degree[i]
            if node_degree > n_node_check:
                continue
            v = degree_dict_test[node_degree]
            degree_dict_test[node_degree] = v + 1
    degree_dict_test = dict(sorted(degree_dict_test.items()))
    degree_dict_test = {int(k): int(v) for k, v in degree_dict_test.items()}

    correct_dict = {}
    correct_dict = defaultdict(lambda: 0, correct_dict)
    total_test_nodes = 0
    for i in range(len(y)):
        node_degree = degree[i]
        if node_degree > n_node_check:
        ...
        if mask[i] == True:
            total_test_nodes += 1
            if pred[i] == y[i]:
                v = correct_dict[node_degree]
                correct_dict[node_degree] = v + 1

    correct_dict = dict(sorted(correct_dict.items()))
    correct_dict = {int(k): v for k, v in correct_dict.items()}
    print(f"correct_dict: {correct_dict}")

    for k in correct_dict:
        per = degree_dict_test[k] - correct_dict[k]

        total_test_nodes = degree_dict_test[k]
        correct_dict[k] = np.round(per / total_test_nodes, 3)

    print(f'degree_dict_test: {degree_dict_test}')
    print(f'correct_dict: {correct_dict}')
    print(f'degree_dict_test: {degree_frequency}')

    print(f'total_test_nodes: {total_test_nodes}')
    with open('degree_alldataset.txt', 'a') as data:
        data.write(str(degree_dict_test) + "\n")
        data.write(str(correct_dict) + "\n")
        data.write(str(degree_frequency) + "\n")
        data.write("============================\n")

    # plot_degree(correct_dict, n_node_check)
...
def plot_degree(data, length, degree_dict_test):
    x1 = np.arange(0, length, 1)
    # y1 = list(degree_dict_test.values())
    ...
    # plt.plot(x1, y1, '-g', label='Number of nodes')
    # plt.plot(x1, values, '-b', label='The performance')
...
def test(model, test_data, mask, device,device_2, g, adj_org, trans_logM, sim, phi, B, degree, k_transition, current_epoch,
    ...
    # pred = logits.max(1)[1].to(device)[mask]
    ...
    if test_node_degree == 1:
        a = (f'acc: {acc}')
        text_file = open("degree_alldataset.txt", "a")
        n = text_file.write(a + "\n")
        text_file.close()
        check_degree(test_data, pred_before, degree, mask )
    return acc, f1
...
def train_finetuning_cluster(model, train_data, mask, optimizer, device,device_2, g, M, trans_logM, sim, phi, B, k_transition,
    ...
    # Identity = torch.eye(g.number_of_nodes())
    # # print(f'identity: {Identity.size()}, C size: {C.size()}')
    # wrt = torch.norm(((torch.mm(C, C_t) - Identity) ** 2)) / n

    # print(f'spectral_loss: {spectral_loss}, collapse_loss: {collapse_loss}, wrt: {wrt}')
...
def test_cluster(model, test_data, mask, optimizer, device,device_2, g,
    ...
    Adj = Adj.todense()
    # print(f'acc: {accuracy_score(y_true, pred)}')
    # print(f'precision: {precision(y_true, pred)}')
    # print(f'recall: {recall(y_true, pred)}')
    # print('----------------')
    # print(f'nmi: {normalized_mutual_info_score(y_true, pred)}')
    # print(f'q: {modularity(Adj, pred)}')
    # print(f'c: {conductance(Adj, pred)}')
