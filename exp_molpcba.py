...
def run(i, dataset_full, num_features, num_classes ):
	...
	return 0
from train_molpcba import train_epoch, evaluate_network, train_epoch_graph_classification, train_epoch_domainadaptation
	...
	for epoch in range(1, args.ft_epoches):
		...
		if epoch % 1 == 0:
			if args.task =="graph_regression":
				print(f'Epoch: {epoch}	|Best_epoch: {best_epoch}	|Train_loss: {np.round( epoch_train_loss, 6)}	|Val_loss: {np.round(epoch_val_loss, 6)}	| Train_ap: {np.round(epoch_train_auc,6)}	| Val_ap: {np.round(epoch_val_auc, 6) }	| epoch_test_ap: {np.round(epoch_test_auc, 6) } ')
		...
		if time.time()-t0 > 48 * 3600:
			...
			break
	_, test_ap = evaluate_network(args, best_model, optimizer, device, test_loader, epoch, batch_size)# (best_model, device, test_loader, epoch)
...
def main():
	...
	if args.dataset in ["ogbg-molpcba", "ogbg-molhiv", "ogbg-ppa","ogbg-code2"]:
		...
		args.num_classes = dataset.num_tasks 		#	dataset.num_classes
	...
	samples_all = []
	num_features = args.num_features
	print(f'num_features: {num_features}, args.num_classes: {args.num_classes} |num_tasks= {dataset.num_tasks} ')
...
def generate_graphs(dataset, k_hop):
	...
	for i in range(len(dataset)):
		...
		try:
			...
			#set_subgraphs = torch.from_numpy(set_subgraphs)
...
if __name__ == '__main__':
	...
	#
	parser.add_argument("--dataset", default="ogbg-molpcba", help="Dataset")
