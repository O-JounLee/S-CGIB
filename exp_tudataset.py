...
from train_tudataset import train_epoch, evaluate_network, train_epoch_graph_classification, \
    train_epoch_domainadaptation
...
def run_epoch_graph_classification(model, train_loader, val_loader, test_loader, num_features, file_name, batch_size):
    ...
    for epoch in range(1, args.ft_epoches):
        ...
        if epoch % 1 == 0:
            print(
                f'Epoch: {epoch}	|Best_epoch: {best_epoch}	|Train_loss: {np.round(epoch_train_loss, 6)}	|Val_loss: {np.round(epoch_val_loss, 6)}	| Train_acc: {np.round(epoch_train_auc, 6)}	| Val_acc: {np.round(epoch_val_auc, 6)}	| epoch_test_acc: {np.round(epoch_test_auc, 6)} ')
    ...
    print("Train ACC: {:.4f}".format(train_auc))
    print("Test ACC: {:.4f}".format(test_auc))
...
def main():
    ...
    if args.dataset in ["ENZYMES", "PROTEINS", "NCI1", "NCI109", "Mutagenicity"]:
        ...
        print("args.dataset in ENZYMES PROTEINS NCI1 NCI109")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--recons_type", default="logM")
    ...
    ################################################################################################	args.k_transition		 python script_run.py
    #		all_subgraphs = [dgl.khop_in_subgraph(g, individual_node, k = args.k_transition)[0] for individual_node in node_ids]
