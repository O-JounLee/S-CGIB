...
from metrics import accuracy_TU, MAE, rmse
...
def train_epoch_graph_classification(args, model, optimizer, device, data_loader, epoch, batch_size):
    ...
    if args.dataset == "FreeSolv":
        evaluator = Evaluator(name="ogbg-molfreesolv")
    elif args.dataset == "ESOL":
        evaluator = Evaluator(name="ogbg-molesol")
    elif args.dataset == "Lipo":
        evaluator = Evaluator(name="ogbg-mollipo")
    else:
        evaluator = None
    epoch_train_auc = 0
    epoch_train_acc = 0
    ...
    for iter, (batch_graphs, batch_targets, batch_subgraphs, _) in enumerate(data_loader):
        ...
        loss = model.loss_RMSE(batch_scores, batch_targets)
        ...
        optimizer.step()
        epoch_loss += loss.detach().item()
        epoch_train_rmse += rmse(batch_scores, batch_targets)
    epoch_loss /= (iter + 1)

    input_dict = {"y_true": targets, "y_pred": scores}
    #epoch_train_rmse = evaluator.eval(input_dict)['rmse']
    #epoch_train_rmse /= (iter + 1)
    # input_dict = {"y_true": targets, "y_pred": scores}
    # epoch_train_auc = evaluator.eval(input_dict)['rmse']
    epoch_train_rmse /= (iter + 1)
    return epoch_loss, epoch_train_rmse, optimizer
...
def evaluate_network(args, model, optimizer, device, data_loader, epoch,
    ...
    with torch.no_grad():
        ...
        for iter, (batch_graphs, batch_targets, batch_subgraphs, _) in enumerate(data_loader):
            ...
            epoch_test_rmse += rmse(batch_scores, batch_targets)

        input_dict = {"y_true": targets, "y_pred": scores}
        #epoch_test_rmse = evaluator.eval(input_dict)['rmse']
        #epoch_test_rmse /= (iter + 1)

        epoch_test_rmse /= (iter + 1)
        epoch_test_loss /= (iter + 1)

    return epoch_test_loss, epoch_test_rmse
